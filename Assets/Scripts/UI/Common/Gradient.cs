...
namespace ETD.Scripts.UI.Common
{
	public class Gradient : BaseMeshEffect
	{
		...
		[Range(0f, 1f)] public float Flexible = 0;

		private VertexHelper _vertexHelper;

		public override void ModifyMesh(VertexHelper vh = null)
		{
			if (enabled && (vh != null || _vertexHelper != null) )
			{
				if (vh != null)
					_vertexHelper = vh;
				...
				for (var i = 0; i < _vertexHelper.currentVertCount; i++)
				{
					_vertexHelper.PopulateUIVertex(ref vertex, i);
					var localPosition = localPositionMatrix * vertex.position;
					vertex.color *= Color.Lerp(Color2, Color1, localPosition.y);
					_vertexHelper.SetUIVertex(vertex, i);
				}
			}
		}
		...
		private Matrix2x3 LocalPositionMatrix(Rect rect, Vector2 dir)
		{
			...
			float c = 0.5f;
			float ax = rectMin.x / rectSize.x + Flexible;
			float ay = rectMin.y / rectSize.y + Flexible;
