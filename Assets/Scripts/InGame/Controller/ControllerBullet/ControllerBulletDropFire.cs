...
{
    public class ControllerBulletDropFire: ControllerBullet
    {
        private readonly ViewBulletDropFire _view; 

        private bool _isDropped;
        private Vector2 dropPosition;
        
        public ControllerBulletDropFire(CancellationTokenSource cts, Transform parent)
            : base(cts, parent,  View.View.Get<ViewBulletDropFire>())
        {
            _view = (ViewBulletDropFire)viewBullet;
            MoveSpeed = 15f;
        }
        ...
        public override async UniTaskVoid Shot(Vector2 endPos, HashSet<IDamageable> nonTargets = null)
        {
            ...
            while (!_isDropped && _view.isActiveAndEnabled)
            {
                ...
                if (Vector2.Distance(Position, dropPosition) <= 0.2f
